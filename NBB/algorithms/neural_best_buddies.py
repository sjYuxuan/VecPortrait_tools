...
class sparse_semantic_correspondence():
    ...
    def save_correspondence_as_txt(self, correspondence, name='', datarootA='', datarootB='', last=False):

        A_true = Image.open(datarootA)
        A_width, A_height = A_true.size
        A_maxside = max(A_width, A_height)

        B_true = Image.open(datarootB)
        B_width, B_height = B_true.size
        B_maxside = max(B_width, B_height)

        A_corres = copy.deepcopy(correspondence[0])
        B_corres = copy.deepcopy(correspondence[1])

        # Do all the work on resized-to-square images but output point coordinates in the original ratio
        for i in range(len(A_corres)):
            A_corres[i][0] = int(A_corres[i][0] * (A_height/A_maxside))
            A_corres[i][1] = int(A_corres[i][1] * (A_width/A_maxside))
            B_corres[i][0] = int(B_corres[i][0] * (B_height/B_maxside))
            B_corres[i][1] = int(B_corres[i][1] * (B_width/B_maxside))

        self.save_points_as_txt(A_corres, 'correspondence_A' + name)
        self.save_points_as_txt(B_corres, 'correspondence_B' + name)

        print('\nReturning {} correspondences in total'.format(len(A_corres)))

        print('\nSaved content points at ' + self.save_dir + '/correspondence_A' + name + '.txt')
        print('Saved style points at ' + self.save_dir + '/correspondence_B' + name + '.txt')
    ...
    def finalize_correspondence(self, correspondence, image_width, L, datarootA, datarootB):
        ...
        draw.draw_correspondence(self.A, self.B, scaled_correspondence, self.draw_radius[L-1], self.save_dir, L, datarootA=datarootA, datarootB=datarootB)
    ...
    def run(self, A, B, datarootA, datarootB):
        ...
        for L in range(L_start, self.L_final-1, -1):
            ...
            if L > self.L_final:
                ...
                draw.draw_correspondence(self.A, self.B, scaled_correspondence, draw_radius, self.save_dir, L, datarootA=datarootA, datarootB=datarootB)
        ...
        filtered_correspondence = self.finalize_correspondence(correspondence, image_width, self.L_final, datarootA, datarootB)
        draw.draw_correspondence(self.A, self.B, filtered_correspondence, self.draw_radius[self.L_final-1], self.save_dir, datarootA=datarootA, datarootB=datarootB)
        self.save_correspondence_as_txt(filtered_correspondence, datarootA=datarootA, datarootB=datarootB)
        with open(os.path.join(self.save_dir, 'correspondence_activation' + '.txt'), 'wt') as opt_file:
